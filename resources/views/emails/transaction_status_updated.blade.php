...
<p>Your order <strong>#{{ $transaction->id }}</strong> status is now: <strong>{{ ucfirst($transaction->status) }}</strong>.</p>
    ...
    <tbody>
        @foreach($transaction->items as $item)
            <tr>
                ...
                <td>{{ optional(optional($item->product)->category)->name ?? 'N/A' }}</td>
